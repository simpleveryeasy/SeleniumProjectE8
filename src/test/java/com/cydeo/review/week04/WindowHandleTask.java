...
Scenario 1)
-  Using the Valet parking lot, set today as the entry date & time and tomorrow as the exit giving a
    full 24 hour window (you can attempt to use the DatePicker).
     */

    /*
    When we have multiple window is opened how we switch to different window
    getWindowHandles()---> set of strings
    driver.switchTo().window(string of window handle)
     */
    ...
    public void windowHandleTest() {
        ...
        //click on the first calender
        driver.findElement(By.xpath("(//img[@src='cal.gif'])[1]")).click();

        //get all the opened window handles
        Set<String> windowHandles = driver.getWindowHandles();

        //get your current window
        String mainWindowHandle = driver.getWindowHandle();

        //to switch to window we aren't at
